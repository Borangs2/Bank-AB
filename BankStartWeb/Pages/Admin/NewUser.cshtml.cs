...
public class NewModel : PageModel
{
    ...
    [BindProperty] public bool EmailConfirmed { get; set; } = true;

    [BindProperty] public bool TwoFA { get; set; }

    [BindProperty] public bool IsCashier { get; set; } = false;

    [BindProperty] public bool IsAdmin { get; set; } = false;
        ...
        if (ModelState.IsValid)
        {
            ...
            switch (result)
            {
                ...
                case IUserService.ReturnCode.InvalidUsername:
                    ModelState.AddModelError(nameof(Name),
                        "Användarnamnet är ogiltigt. Användarnamnet kan bara innehålla bokstäver mellan a-z och siffror från 0-9");
                    break;
                case IUserService.ReturnCode.InvalidPassword:
                    ModelState.AddModelError(nameof(Password),
                        "Lösenordet är ogiltigt. Lösenordet behöver innehålla åtminstone 1 stor och lite bokstav och minst 1 siffra");
                    break;
